...
class AgentV1DecisionTests(unittest.TestCase):
    ...
    def test_build_contract_uses_manual_fallback_when_bridge_unavailable(self) -> None:
                ...
                json.dumps(
                    {
                        ...
                        "robinhood_mcp_handoff": {
                            "bridge_status": {"available": False, "status": "disabled"},
        ...
        self.assertEqual(contract["broker_integration_status"], "disabled")
        self.assertIn("broker_integration_unavailable", contract["risk_flags"])
        self.assertFalse(contract["permissions"]["read_account_status"])
        self.assertFalse(contract["permissions"]["monitor_quotes"])
        self.assertEqual(contract["decision"], "operator_confirm_required")
