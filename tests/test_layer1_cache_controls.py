...
class Layer1CacheControlTests(unittest.TestCase):
    ...
    def test_fred_macro_is_fresh_when_latest_overlay_within_lag(self) -> None:
        ...
        with (
            patch.object(fred, "MAX_LAG_DAYS", 5),
            patch.object(fred, "FORCE_REFRESH", False),
            patch.object(
    ...
    def test_options_snapshots_to_process_skips_existing_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                with (
                    patch.object(options_metrics, "UNDERLYING", "SPY"),
                    patch.object(options_metrics, "DTE_MIN", 0),
                    patch.object(options_metrics, "DTE_MAX", 1),
                    patch.object(options_metrics, "ONLY_SNAPSHOT_TS", ""),
