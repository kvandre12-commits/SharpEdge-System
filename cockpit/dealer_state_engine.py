...
def _wall_state(spot: float | None, call_wall: Any, put_wall: Any) -> dict[str, Any]:
    call_dist = (
        _pct_distance(spot, float(call_wall))
        if isinstance(call_wall, (int, float))
        else None
    )
    put_dist = (
        _pct_distance(spot, float(put_wall))
...
def _pin_state(spot: float | None, pin: Any) -> dict[str, Any]:
    ...
    pin_dist = _pct_distance(spot, float(pin))
...
def _gamma_state(gp: dict[str, Any] | None) -> dict[str, Any]:
    ...
    quality = str(data.get("gamma_data_quality") or "ok").lower()
    if quality != "ok" or regime not in {"positive", "negative"}:
        return {
            "state": "gamma_unknown",
            "regime": regime,
            "quality": quality,
            "reason": "gamma data quality is weak or unknown",
        }
    if regime == "positive":
        return {
            "state": "gamma_damping",
            "regime": regime,
            "quality": quality,
            "reason": "positive gamma dampens directional follow-through",
        }
    return {
...
def build_dealer_state(
    ...
    if gamma_name == "gamma_damping" and (
        ...
        return {
            **packet,
            ...
            "bias": wall_bias,
            "reason": f"positive gamma pinning: {pin_reason}; {wall_reason if wall_state.get('state') != 'no_near_wall' else 'pin/chop risk'}",
        }
    ...
    return {
        **packet,
        ...
        "bias": wall_bias,
        "reason": f"negative gamma supports expansion; {wall_reason if wall_state.get('state') != 'no_near_wall' else 'accepted breaks can run'}",
    }
