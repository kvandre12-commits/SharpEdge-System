...
def render_market_day_block(market_day: dict[str, Any], flow: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"BUCKET BRAIN: {_esc(bucket)} / {score} / {_esc(bias)}</div>"
...
def render_bucket_conditioned_spine_block(spine: dict[str, Any]) -> str:
    ...
    action = spine.get("recommended_action") or "watch_only"
    ...
    return (
        ...
        f"BUCKET-CONDITIONED SPINE: {_esc(gate)} / {spine.get('score', 0)} / {_esc(bias)}</div>"
        f'<div style="color:{FG};font-size:12px;margin-top:5px">'
        f"Action: {_esc(action)} • bias strength {_esc(spine.get('bias_strength', 0))}</div>"
        f'<div style="color:{MUTE};font-size:11px;margin-top:4px;{WRAP}">'
...
def render_authority_adjudication_block(packet: dict[str, Any]) -> str:
    if not packet:
        return ""
    doing = packet.get("we_are_doing_this") or {}
    ...
    return (
        ...
        f'<div style="color:{accent};font-weight:bold;font-size:13px">AUTHORITY ADJUDICATION • { _esc(engine.upper()) } PRIMARY</div>'
        ...
        f'<div style="padding:8px;border:1px solid #30363d;border-radius:6px;background:#161b22">'
        f'<div style="color:{MUTE};font-size:11px">This may be occurring</div>'
        ...
        f'<div style="padding:8px;border:1px solid #30363d;border-radius:6px;background:#161b22">'
        f'<div style="color:{MUTE};font-size:11px">But we are doing this</div>'
