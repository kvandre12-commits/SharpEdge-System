...
ATTEMPT_PATH = ROOT / "phone_companion/launchers/android_signal_import_attempt.json"
RESULT_PATH = ROOT / "phone_companion/launchers/android_signal_import_result.json"
...
def _load_minified_signal(path: Path) -> str:
...
def build_share_command(signal_json_text: str) -> list[str]:
    return [
        "am",
        "start",
        "-S",
...
def launch_signal_to_android_viewer(
    ...
    *,
    ...
    export_proof = export_signal(
        signal_path, android_root, proof_path, live_import_path
    )
    signal_json_text = _load_minified_signal(live_import_path)
    installed = app_installed()
    command = build_share_command(signal_json_text)

    attempt = {
        "artifact_type": "sharpedge_android_signal_import_attempt",
        "status": "starting",
        ...
        "payload_bytes": len(signal_json_text.encode("utf-8")),
        "command_preview": command[:-1] + [f"<json:{len(signal_json_text)} chars>"],
        "dry_run": dry_run,
    }
    ...
    if not installed:
        result = {
            **attempt,
            "artifact_type": "sharpedge_android_signal_import_result",
...
def main(argv: list[str]) -> int:
    ...
    result = launch_signal_to_android_viewer(dry_run=dry_run)
